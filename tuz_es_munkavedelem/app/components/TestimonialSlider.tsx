...
const accentColor = {
  starActive: 'text-orange-500', // Brand narancs a csillagokhoz
  quoteIcon: 'text-orange-200',   // Nagyon halvány narancs az idézet ikonhoz
  quoteIconOpacity: 'opacity-60',
};
  ...
  return (
    <div className="h-full bg-white rounded-2xl shadow-xl p-8 flex flex-col justify-between border border-gray-100 overflow-hidden">
      <div className="relative z-10 text-center flex flex-col flex-grow">
        {/* MÓDOSÍTVA: Idézet ikon színe */}
        <ChatBubbleLeftRightIcon className={`absolute -top-2 -left-2 w-14 h-14 sm:w-16 sm:h-16 ${accentColor.quoteIcon} ${accentColor.quoteIconOpacity}`} />
        <div className="relative flex-grow flex flex-col">
          <div className="flex justify-center mb-4">
            {[...Array(5)].map((_, i) => (
              // MÓDOSÍTVA: Csillagok színe
              <StarIcon key={i} className={`w-5 h-5 ${i < (testimonial.rating || 0) ? accentColor.starActive : 'text-gray-300'}`} />
            ))}
          </div>
    ...
    <section className="py-16 lg:py-24 bg-gradient-to-br from-stone-50 via-amber-50 to-stone-100 overflow-hidden">
      <div className="container mx-auto">
        ...
        <div className="max-w-6xl mx-auto overflow-hidden">
          <motion.div
            className="flex"
            animate={{ x: ["0%", `-${100 * (testimonials.length / (extendedTestimonials.length / testimonials.length))}%`] }} // Ezt a logikát meghagytam, feltételezve, hogy tesztelve lett
            transition={{
              ease: "linear",
          ...
          >
            {extendedTestimonials.map((testimonial, index) => (
              <div key={testimonial.id + '-' + index} className="flex-shrink-0 w-full sm:w-1/2 md:basis-1/3 p-2 sm:p-3">
