                        ...
                        {!isSubmitted ? (
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="space-y-4 mb-6">
                                    <div className="relative group"><div className={`absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none`}><UserIcon className="h-5 w-5 text-gray-400 group-focus-within:text-cyan-500" /></div><input type="text" name="name" value={formData.name} onChange={handleFormChange} className="block w-full text-gray-500 pl-10 text-black pr-3 py-3 border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500" placeholder="Teljes Név*" required /></div>
                                    <div className="relative group"><div className={`absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none`}><EnvelopeIcon className="h-5 w-5 text-gray-400 group-focus-within:text-cyan-500" /></div><input type="email" name="email" value={formData.email} onChange={handleFormChange} className="block w-full text-gray-500 pl-10 pr-3 py-3 border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500" placeholder="Email cím*" required /></div>
                                    <div className="relative group"><div className={`absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none`}><PhoneIcon className="h-5 w-5 text-gray-400 group-focus-within:text-cyan-500" /></div><input type="tel" name="phone" value={formData.phone} onChange={handleFormChange} className="block w-full text-gray-500 pl-10 pr-3 py-3 border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500" placeholder="Telefonszám" /></div>
                                </div>
                                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 mb-10">
                                    {servicesList.map((service, index) => (
                                        <motion.li key={service.id} className="flex items-start gap-3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 + index * 0.05 } }}>
                                            <input type="checkbox" id={`service-${service.id}`} checked={selectedServices.includes(service.id)} onChange={() => handleCheckboxChange(service.id)} className={`form-checkbox h-6 w-6 rounded border-gray-300 text-cyan-500 shadow-sm focus:ring-cyan-500 cursor-pointer`} />
        ...
        <>
            ...
            <div style={{ backgroundColor: '#ffffff', backgroundImage: `linear-gradient(rgba(3, 186, 190, 0.15) 1px, transparent 1px), linear-gradient(to right, rgba(3, 186, 190, 0.15) 1px, transparent 1px)`, backgroundSize: '4rem 4rem' }} className="font-['Poppins',_sans_serif] min-h-screen relative">
                ...
                <motion.section ref={introSectionRef} id="bemutatkozas" className="py-24 lg:py-32 px-4 sm:px-6 lg:px-8" variants={introSectionVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
                    ...
                    <motion.h2 className="text-4xl lg:text-5xl font-black text-slate-900 tracking-normal leading-tight mb-12 text-center" variants={introItemVariants}>Ezt a három alapvető dolgot ismerned kell ahhoz, <span className='text-cyan-400'>hogy biztonságban tudd a vállalkozásodat!</span></motion.h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-16 w-full">
                            <motion.div variants={introItemVariants} whileHover={{ y: -8, scale: 1.03 }} className="relative bg-white/50 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-slate-200/80 transition-all duration-300 hover:shadow-2xl">
                                ...
                                <p className="text-slate-600 leading-relaxed">Többről van szó mint a tűzoltó készülék meglétéről. A Tűzvédelem egy proaktív stratégia, ami a probléma megelőzésére, és gyors reagálására szolgál. <strong>Fontos:</strong> Kötelező, ha rendelkezel bármilyen telephellyel!</p>
                            </motion.div>
                             <motion.div variants={introItemVariants} whileHover={{ y: -8, scale: 1.03 }} className="relative bg-white/50 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-slate-200/80 transition-all duration-300 hover:shadow-2xl">
                                <motion.div className="absolute w-24 h-24 -top-9 -right-9" animate={{ y: [-4, 4, -4], rotate: [-3, 3, -3], }} transition={{ duration: 8, repeat: Infinity, repeatType: 'mirror', ease: 'easeInOut' }}><FaHelmetSafety className="w-full h-full text-yellow-500 drop-shadow-lg" /></motion.div>
                                <motion.div variants={iconCardVariants} className="flex items-center gap-5 mb-5"><div className={`p-4 rounded-xl bg-gradient-to-br from-amber-400 to-yellow-500`}><FaHelmetSafety className="w-8 h-8 text-white" /></div><h3 className="text-2xl font-bold text-slate-800">Munkavédelem</h3></motion.div>
                                <p className="text-slate-600 leading-relaxed">Munkaadóként felelősséggel tartozol a kollégák iránt, és ezt a hatóságok is így gondolják.<br /><span className="font-bold">Fontos:</span> A “legveszélytelenebb” munkahelyeken is elvárják az alapvető munkavédelmi előírások betartását, hiszen az emberi élet érték.</p>
                            </motion.div>
                            <motion.div variants={introItemVariants} whileHover={{ y: -8, scale: 1.03 }} className="relative bg-white/50 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-slate-200/80 transition-all duration-300 hover:shadow-2xl">
